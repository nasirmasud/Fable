...
import { FaBook, FaBookmark, FaChartLine, FaHistory, FaHome, FaMoneyBill, FaUsers } from "react-icons/fa";
...
const DashboardItems = {
  reader: [
    ...
    { icon: GiBookshelf, label: "All Books", link: "/all-books" },
    ...
    <div className="border-t border-white/10 p-4 bg-white/2">
      ...
      <div className="mt-4 flex flex-col gap-2">
        <Link
          ...
          className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-white/5 hover:text-white"
        >
          <User size={16} /> Profile
        </Link>
        <button
          onClick={handleLogout}
          className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-400 hover:bg-red-500/10"
        >
    ...
    <>
      ...
      <aside
        className={`${isOpen ? "translate-x-0" : "-translate-x-full"
          } md:translate-x-0 fixed top-0 left-0 h-full w-64 bg-[#070314] border-r border-white/10 z-50 flex flex-col transition-transform duration-300`}
      >
