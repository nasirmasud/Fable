...
function StatusBadge({ status }) {
  ...
  return (
    <Badge variant="outline" className={`text-xs px-2 py-0 border-0 ${map[status] || "bg-gray-500/15 text-gray-400"}`}>
      {status}
    </Badge>
  );
}
...
function ActionMenu({ user, onAction, pending }) {
  ...
  return (
    <div className="flex items-center justify-center w-8 h-8">
      {showConfirm ? (
        <div className="flex items-center gap-1 bg-[#1a1a35] border border-white/10 p-1 rounded-full shadow-xl animate-in fade-in zoom-in duration-200">
          <button
            onClick={() => { onAction(user); setShowConfirm(false); }}
            className="w-6 h-6 flex items-center justify-center text-red-400 hover:bg-red-500/20 rounded-full transition-colors"
          >
            <Check size={14} />
          </button>
          <button
            onClick={() => setShowConfirm(false)}
            className="w-6 h-6 flex items-center justify-center text-gray-400 hover:bg-white/10 rounded-full transition-colors"
          >
            <X size={14} />
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#0d0d1a] text-white p-6 space-y-6">
      ...
      <Card className="bg-[#13132a] border-white/5 rounded-2xl">
        <CardHeader className="px-6 pt-6 pb-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <input
              ...
              value={search}
              onChange={(e) => { setSearch(e.target.value); setPage(1); }}
              className="w-full sm:w-56 h-9 pl-4 bg-[#0d0d1a] border border-white/10 rounded-xl text-sm focus:ring-1 focus:ring-purple-500"
            />
            <div className="flex items-center gap-2">
              <Select value={roleFilter} onValueChange={(v) => { setRoleFilter(v); setPage(1); }}>
                <SelectTrigger className="w-28 h-9 bg-[#0d0d1a] border-white/10 text-xs text-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#1a1a35] border-white/10 text-white">
        ...
        <CardContent className="px-6 pb-6">
          <div className="grid grid-cols-[2fr_1.5fr_80px_80px_90px_90px_36px] gap-3 text-xs text-gray-500 uppercase pb-3 border-b border-white/5">
            <span>User</span><span>Email</span><span>Role</span><span>Status</span><span>Books</span><span>Joined</span><span />
          </div>
          <div className="divide-y divide-white/[0.04]">
            {paginated.map((user) => (
              <div key={user.id} className="grid grid-cols-[2fr_1.5fr_80px_80px_90px_90px_36px] gap-3 items-center py-3">
                <p className="text-sm font-medium text-white truncate">{user.name}</p>
                <p className="text-sm text-gray-400 truncate">{user.email}</p>
                <RoleBadge role={user.role} />
                <StatusBadge status={user.status} />
                <p className="text-sm text-gray-300">{user.books || "—"}</p>
                <p className="text-xs text-gray-500">{user.joined}</p>
                <ActionMenu user={user} onAction={handleAction} pending={pendingIds.has(user.id)} />
              </div>
            ))}
